...
class LocateChangesWalk(LocateChangesStrategy):
    ...
    def __init__(self, branch="HEAD", upstream="upstream/master",
                 search_ref=None, *args, **kwargs):

        if not search_ref:
            search_ref = upstream
        self.searcher = UpstreamMergeBaseSearcher(branch=upstream,
                                                  pattern=search_ref)
...
@subcommand.arg('--search-refs', metavar='<pattern>', default='upstream/*',
                help='Refs to search for previous import commit')
